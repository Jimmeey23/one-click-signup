...
function ShareRoutePage() {
    ...
    return (
      <div className="min-h-screen px-6 py-16">
        <div className="mx-auto max-w-xl rounded-2xl border border-border bg-card p-8">
          ...
          <p className="mt-3 text-sm text-muted-foreground">The shared URL looks invalid or incomplete.</p>
          <div className="mt-6">
            <Button asChild>
  ...
  const signupUrl = `/signup/${shareId}`;
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <div className="rounded-3xl border border-border bg-card p-8 shadow-sm">
          ...
          <p className="mt-3 text-sm text-muted-foreground">
            {(payload.eventDate || "No date") + (payload.eventTime ? ` · ${payload.eventTime}` : "")}
            {" · "}
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-2">
            ...
            <Detail label="Kids consent" value={payload.includeKidsConsent ? "Included" : "Not included"} />
          ...
          {payload.otherDetails ? (
            <div className="mt-6 rounded-2xl bg-secondary p-4 text-sm leading-relaxed">
              <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Other key details</p>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Button asChild>
              <a href={`mailto:?subject=${encodeURIComponent(payload.eventName)}&body=${encodeURIComponent(fullUrl)}`}>Share via email</a>
            </Button>
            <Button
              variant="secondary"
              onClick={async () => {
            ...
            <Button asChild>
              <Link to={signupUrl as never}>Open signup form</Link>
            </Button>
